...
Sys.setenv(PROJ_LIB = "C:\\Users\\ygrund\\AppData\\Local\\R\\win-library\\4.4\\PROJ\\proj")
...
# day_start <- "2025001" # when a new year starts
...
# r.download_file_list = ['2024183', '2024184', '2024185', '2024186', '2024187', '2024188']
# r.download_file_list = ['2024188']
for raw_name in r.download_file_list:
  arcpy.CheckOutExtension("Spatial")
  # test: raw_name = '2024323'
  print(raw_name)
...
zonal_combined <- bind_rows(all_data) %>% 
  ...
  dplyr::mutate(`7DGMDM` = as.numeric(NA),
                `7DADM` = as.numeric(NA),
...
dta2 <- readxl::read_excel(paste0(data_path,"cyan_ci_2025.xlsx")) %>% 
  dplyr::filter(GNISIDNAME %in% dta1$inApp) # filter out saline lakes

dta3 <- readxl::read_excel(paste0(data_path,"cyan_ci_2002-2024.xlsx")) %>% 
  dplyr::filter(GNISIDNAME %in% dta1$inApp) # filter out saline lakes
...
tbl.data <- dta_wide %>% 
  ...
  dplyr::mutate(`7DGMDM` = ifelse(`7DGMDM`<= 6310, "Non-detect",format(round(`7DGMDM`,0),big.mark=",",scientific = FALSE))) %>%
  ...
  dplyr::rename(`Waterbody_GNISID*`= GNISIDNAME,
                ...
                `7DGMDM (cells/mL)` = `7DGMDM`,
                `Date_7DGMDM` = Date)
  
map.tbl.data <- tbl.data %>% 
  dplyr::filter(!`7DGMDM (cells/mL)`== "Non-detect") %>% 
  dplyr::mutate(`7dgmdm` = gsub(",","",`7DGMDM (cells/mL)`)) %>%
  dplyr::filter(as.numeric(`7dgmdm`) > 100000) %>%
  dplyr::select(`Waterbody_GNISID*`,Basin,`7DGMDM (cells/mL)`,`7DADM (cells/mL)`,`Date_7DGMDM`,`Days of Data`,-`7dgmdm`)
...
```{r save_7-day_table}
# Save the 7day table ----
excel_file <- file.path("//deqhq1/WQ-Share/Harmful Algal Blooms Coordination Team/CyAN_Weekly_Report/Images/2024", "report7days.xlsx")
# first time when creating the excel
# create_excel <- function(data, excel_file) {
#   wb_7d <- openxlsx::createWorkbook()
#   openxlsx::addWorksheet(wb_7d, "Sheet1")
#   data$Date <- Sys.Date()
#   openxlsx::writeData(wb_7d, "Sheet1", data)
#   openxlsx::saveWorkbook(wb_7d, excel_file, overwrite = TRUE)
# }
# create_excel(map.tbl.data, excel_file) 

wb_7d <- openxlsx::loadWorkbook(excel_file)
existing_data <- readxl::read_excel(excel_file, sheet = 1) %>% 
  dplyr::mutate(Date_7DGMDM = as.Date(Date_7DGMDM),
                Date = as.Date(Date))
new_data <- map.tbl.data
new_data$Date <- Sys.Date()
colnames(new_data) <- colnames(existing_data)
updated_data <- dplyr::bind_rows(existing_data, new_data)
openxlsx::writeData(wb_7d, "Sheet1", updated_data, startCol = 1, startRow = 1)
openxlsx::saveWorkbook(wb_7d, excel_file, overwrite = TRUE)
