...
data_chla <- readxl::read_excel(paste0(data_path,"cyan_chla_2026.xlsx"))
data_field <- readxl::read_excel(paste0(data_path,"field_data_2026.xlsx"))
...
field_lakes <- readxl::read_excel(paste0(data_path,"Resolvable_Lakes.xlsx"), sheet = "field")
```

## Step 2. Download data from [NASA's data site](https://oceandata.sci.gsfc.nasa.gov/directdataaccess/Level-3%20Mapped%20with%20GEO/Merged-S3-CYAN/).
...
saveRDS(download_file_list, file = "download_file_list.rds")
...
# download_file_list <- readRDS("download_file_list.rds")
...
python_list <- paste0("download_file_list = [",
                      ...
                      "]")
# python_list <- "download_file_list = [\"2026204\", \"2026205\", \"2026206\", \"2026207\"]"
...
```{r zonal_chl_tbl}
# download_file_list <- c(paste0("2026", sprintf("%03d", 1:4)))
...
writexl::write_xlsx(zonal_update, paste0(data_path,"cyan_chla_2026.xlsx"))

print("Step 3.4 - Chl-a table is completed")

```

## Step 4. Download AWQMS data.

```{r awqms_data}
field_data <- AWQMSdata::AWQMS_Data(
  Char_Name = c(
    'Anatoxin-A',
    'Chlorophyll a',
    'Cylindrospermopsin',
...
writexl::write_xlsx(field_data, paste0(data_path,"field_data_all_2026.xlsx"))

field_data_clean <- field_data %>%
  dplyr::left_join(field_lakes, by = "AU_ID") %>%
  dplyr::select(MLocID,StationDes,Lat_DD,Long_DD,SampleStartDate,Char_Name,Result_Operator,Result_Numeric,Result_Unit,GNIS_Name_ID,Result_status,DQL,QualifierAbbr,Result_Type,MonLocType,`Project1`) %>% 
  dplyr::filter(!is.na(GNIS_Name_ID)) %>% 
  dplyr::filter(Result_status %in% c("Final","Accepted","Validated")) %>% # excluded: "Preliminary" and "Rejected" 
  dplyr::filter(DQL %in% c("A","B","E",NA)) %>% # excluded: "C" and "D" 
  dplyr::filter(is.na(QualifierAbbr)) %>% # excluded: "FQC", "SUS", "H" and "Q"     
  dplyr::filter(Result_Type %in% "Actual") %>% # excluded: "Estimated"
  dplyr::filter(`Project1` %in% c(
    "Harmful Algal Bloom Monitoring",
    "Drinking Water Source Monitoring",
    "ODEQVolMonWQProgram",
    "Surface Water Ambient Monitoring",
    "Total Maximum Daily Load Sampling",
    "Water Quality Response Monitoring",
    "Unknown"
  )) 

writexl::write_xlsx(field_data_clean, paste0(data_path,"field_data_2026.xlsx"))

print("Step 4 - AWQMS is completed")

```

## Step 5. Gether data for the shiny app.
...
# CyAN Chla ----
dta2 <- readxl::read_excel(paste0(data_path,"cyan_chla_2026.xlsx")) %>% 
  dplyr::filter(GNISIDNAME %in% dta1$inApp) # filter out saline lakes

dta3 <- readxl::read_excel(paste0(data_path,"cyan_chla_2016-2025.xlsx")) %>% 
  dplyr::filter(GNISIDNAME %in% dta1$inApp) # filter out saline lakes
...
dta_cyan <- rbind(dta2,dta3) %>% 
  ...
  dplyr::select(GNISIDNAME,Date,Year,Month,Day,Julian_day,`Days of Data`,Parameter,`Detection Qualifier`,Value,Unit,`Result Status`,`Data Source`,Lat_DD,Long_DD,wi_DWSA) %>% 
  dplyr::filter(Year > 2015)

# Field data ----
dta_field_1 <- readxl::read_excel(paste0(data_path,"field_data_2026.xlsx"))

dta_field_2 <- readxl::read_excel(paste0(data_path,"field_data_1999-2025.xlsx"))

dta_field <- rbind(dta_field_1,dta_field_2) %>% 
  dplyr::select(MLocID,StationDes,Lat_DD,Long_DD,SampleStartDate,Char_Name,Result_Operator,Result_status,Result_Numeric,Result_Unit,GNIS_Name_ID) %>% 
  ...
  dplyr::mutate(
    ...
    `Days of Data` ="",
    Parameter = Char_Name,
    `Detection Qualifier` = Result_Operator,
...
field_stations <- rbind(dta_field_1,dta_field_2) %>% 
...
# Advisories ----
dta_adv_1 <- readxl::read_excel(paste0(data_path,"OHA_advisories_2007-2025.xlsx"), sheet = "Advisories")

dta_adv_2 <- readxl::read_excel(paste0(data_path,"OHA_advisories_2026.xlsx"), sheet = "Advisories") %>% 
  dplyr::mutate(Details = dplyr::if_else(is.na(Lifted),"Active",Details)) %>% 
  dplyr::mutate(Lifted = dplyr::if_else(is.na(Lifted),as.POSIXct(Sys.Date()-1),Lifted)) 

advisories <- rbind(dta_adv_1,dta_adv_2) %>% 
  dplyr::filter(Status_OHA == "Advisory") %>% 
  ...
  dplyr::mutate(
    ...
    Lifted_label = if_else(Advisory_status %in% "Active","Not Yet Lifted",format(as.Date(Lifted), "%b %d, %Y"))) %>% 
  dplyr::select(GNIS_Name_ID,Issued, Lifted, Lifted_label,`Dominant genus/toxin`,`Cell Count/Toxin`,Unit)
...
forecast <- readxl::read_excel(paste0(data_path,"forecast.xlsx")) %>% 
  ...
  dplyr::filter(GNIS_Name_ID %in% dta1$inApp) # filter out saline lakes
...
map.tbl.data <- dta_wide %>% 
  ...
  dplyr::filter((!is.na(`Weekly Mean Daily Max (μg/L)`) & as.numeric(`Weekly Mean Daily Max (μg/L)`) >= 24) | 
                   as.numeric(`% Chance of CyanoHAB_map`) >= 50) %>%
  ...
  # dplyr::filter(!`Waterbody_GNISID*` %in% c("Summit Lake_01150624")) # 20260408
...
make_timeseries_plot <- function(df,
                                 df_temp,
                                 ...
                                 add_advisory_legend = TRUE,
                                 add_reporting_legend = TRUE,
                                 plot_title = NULL
) {
  
  p <- plotly::plot_ly()
  
  # Add main lines+markers
  p <- p %>%
    plotly::add_trace(
      data = df %>% dplyr::filter(Parameter %in% parameters_line),
      x = ~as.Date(Date), 
      y = ~Value,
      split = ~Parameter,
      type = "scatter",
  ...
  # Add Chlorophyll-a markers
  if (!is.null(parameter_chla)) {
    p <- p %>%
      plotly::add_trace(
        data = df %>% dplyr::filter(Parameter %in% parameter_chla),
        x = ~as.Date(Date), 
        y = ~Value,
        type = "scatter",
        mode = "markers",
        name = paste0(parameter_chla, " (Field)"),
        marker = list(color = pal.plot[[parameter_chla]], size = 14),
        legendgroup = "chla"
      )
  }
  
  # Add Cyanotoxin markers
  if (!is.null(parameter_toxins)) {
    p <- p %>%
      plotly::add_trace(
        data = df %>% dplyr::filter(Parameter %in% parameter_toxins),
        x = ~as.Date(Date),
        y = ~Value,
        type = "scatter",
        mode = "markers",
  ...
  # Add horizontal guideline lines (e.g. 24 for Chl-a, 15 and 8 for Toxins)
  for (line in horizontal_lines) {
    p <- p %>%
      plotly::add_trace(
        ...
        y = line$y,
        mode = "lines",
        line = list(shape = 'spline', color = line$color, width = 1.5),
        name = line$name,
  ...
  # Add dummy bars for legends
  if (add_advisory_legend) {
    p <- p %>% plotly::add_trace(
      x = c(min(df$Date) - 10000),
      y = c(1),
      type = "bar",
  ...
  # Add advisory markers (up triangle)
  if (!is.null(advisory_markers)) {
    p <- p %>%
      plotly::add_trace(
        data = advisory_markers,
        x = ~x,
        y = ~y,
        text = ~label,
        type = "scatter",
        mode = "markers",
        hoverinfo = "text",
        marker = list(size = 10, symbol = "triangle-up", color = "#D3D3D3"),
        name = "Advisory",
        showlegend = FALSE
      )
  }
  
  # Add report markers (down triangle)
  if (!is.null(report_markers)) {
    p <- p %>%
      plotly::add_trace(
        data = report_markers,
        x = ~x,
        ...
        mode = "markers",
        hoverinfo = "text",
        marker = list(size = 10, symbol = "triangle-down", color = "#D3D3D3"),
        name = "Reporting Period:",
        showlegend = FALSE
      )
  }
  ...
  p <- p %>%
    plotly::layout(
      ...
      xaxis = list(
        title = "Date",
        range = c(as.Date(min(df_temp$Date)) - range_padding, as.Date(max(df_temp$Date)) + range_padding)
      ),
      yaxis = list(
        type = type,
        title = yaxis_title,
