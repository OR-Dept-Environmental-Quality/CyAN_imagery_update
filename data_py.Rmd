...
map.tbl.data <- tbl.data %>% 
  ...
  dplyr::left_join(forecast, by = c("Waterbody_GNISID*"="GNIS_Name_ID")) %>% 
  dplyr::filter(as.numeric(`7dgmdm`) >= 24 | `% Chance of CyanoHAB` >= 12) %>%
  dplyr::select(`Waterbody_GNISID*`,Basin,`7DGMDM (μg/L)`,`7DADM (μg/L)`,`Date_7DGMDM`,`Days of Data`,-`7dgmdm`,`% Chance of CyanoHAB`)
