...
data_pre <- data %>% dplyr::mutate(Julian_day = sprintf("%03d", as.numeric(Julian_day))) 
zonal_combined <- bind_rows(all_data) %>% 
  ...
  dplyr::bind_rows(data_pre[which(as.Date(data_pre$Date)>as.Date(max(sort(unique(data_pre$Date))))-14),]) %>% 
...
zonal_update <- dplyr::bind_rows(data,zonal) %>% dplyr::mutate(Date = as.Date(Date))
...
map.tbl.data <- tbl.data %>% 
  ...
  # dplyr::select(`Waterbody_GNISID*`,Basin,`7DDMGM (cells/mL)`,`7DADM (cells/mL)`,`Days of Data`,-`7dadm`)
