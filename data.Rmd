...
# r.download_file_list = ['2024096']
for raw_name in r.download_file_list:
  arcpy.CheckOutExtension("Spatial")
  # test: raw_name = '2024096'
  print(raw_name)
  ...
  print("...Copy the CI raster to the project folder")
  pattern = os.path.join(ci_path, raw_name + ".*")
  files_to_copy = glob.glob(pattern)
  for file_path in files_to_copy:
    file_name = os.path.basename(file_path)
    base_name, extension = file_name.rsplit('.', 1)
    copy_ci_path = os.path.join(project_path, str(year), str(file_name))
...
dta3 <- readxl::read_xlsx(paste0(data_path,"HAB_resolvablelakes_2002-2023.xlsx")) %>% 
  dplyr::filter(GNISIDNAME %in% dta1$inApp) # filter out saline lakes
...
# save.image(file = "//deqhq1/WQ-Share/Harmful Algal Blooms Coordination Team/HAB_Shiny_app/data.RData")
...
tbl.data <- tbl.data.7days %>% 
  ...
  dplyr::summarise(mean_7DayMax = mean(MAX),                # 7 day average daily maximum
                   geomean_7DayMax = exp(mean(log(MAX))),   # 7-Day Daily Maximum Geometric Mean
                   #`7DMC` = max(MAX),                      # 7 day maximum composite
                   n=n()) %>%    
  dplyr::ungroup() %>% 
  ...
  # dplyr::arrange(desc(geomean_7DayMax)) %>% 
  dplyr::mutate(geomean_7DayMax = ifelse(geomean_7DayMax<= 6310, "Non-detect",
                                      format(round(geomean_7DayMax,0),big.mark=",",scientific = FALSE))) %>% 
  #rbind(no.data) %>% 
  dplyr::left_join(lakes.resolvable, by = "GNISIDNAME") %>% 
  dplyr::mutate(Basin = ifelse(`HU_6_NAME` == "Willamette",`HU_8_NAME`,`HU_6_NAME`)) %>% 
  dplyr::select(GNISIDNAME,Basin,mean_7DayMax,geomean_7DayMax,n) %>%
  # dplyr::select(GNISIDNAME,Basin,geomean_7DayMax,n) %>% 
  ...
  dplyr::rename(`Waterbody_GNISID*` = GNISIDNAME,
                ...
                `7DDMGM (cells/mL)` = geomean_7DayMax,
                `Days of Data` = n)
...
for (x in 1:length(waterbody.list)){
  ...
  for(y in 1:length(image_dates)){
    ...
    df.map.date <- lookup.date %>% dplyr::filter(Date %in% as.Date(image_dates[y]))
    
    tif.dir <- paste0("//deqhq1/WQ-Share/Harmful Algal Blooms Coordination Team/HAB_Shiny_app/data/", df.map.date$Year.dates, "/")
    file.name <- paste0(df.map.date$CyAN_File_NUM,".tif")
    
    if (file.exists(paste0(tif.dir,file.name))){
