...
# data_chla_all <- readxl::read_excel(paste0(data_path,"cyan_chla.xlsx"))
# data_chla <- data_chla_all %>% dplyr::filter(Year == current_year)
...
map.tbl.data <- dta_wide %>% 
  ...
  #               `7DMDM` = ifelse(`7DMDM` <= 0, "Non-detect", format(round(`7DMDM`, 0), big.mark = ",", scientific = FALSE))) %>%
  ...
  dplyr::filter(!`Waterbody_GNISID*` %in% c("Green Peter Lake_01158878")) 
